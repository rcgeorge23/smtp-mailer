...
sudo docker run -d --restart unless-stopped -e VIRTUAL_HOST=smtptrap.net -e FAKE_SMTP_HOST=app.smtptrap.net -e FAKE_SMTP_PORT=25 -e VIRTUAL_PORT=9090 -e SERVER_PORT=9090 --log-driver=syslog -p9090:9090 -t dockernovinet/smtp-mailer-brochureware
sudo docker run -d --restart unless-stopped -e VIRTUAL_HOST=app.smtptrap.net -e VIRTUAL_PORT=8080 -e SERVER_PORT=8080 --log-driver=syslog -p8080:8080 -p25:8025 -t dockernovinet/smtp-mailer
